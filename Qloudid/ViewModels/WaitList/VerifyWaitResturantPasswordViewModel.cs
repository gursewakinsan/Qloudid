...
{
	public class VerifyWaitResturantPasswordViewModel : BaseViewModel
	{
		#region Constructor.
		public VerifyWaitResturantPasswordViewModel(INavigation navigation)
		{
		...
		private async Task ExecuteVerifyInterAppPasswordCommand()
		{
			if (!string.IsNullOrWhiteSpace(Password))
			{
				...
				else if (response.Result == 1)
				{
					...
					int addGuestInfoResponse = await waitService.AddGuestInfoAsync(SubmitWaitResturantDetail);
		...
		public Models.SubmitWaitListResturantDetail SubmitWaitResturantDetail { get; set; }
		#endregion
