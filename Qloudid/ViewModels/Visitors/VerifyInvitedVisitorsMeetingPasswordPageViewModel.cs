...
{
	public class VerifyInvitedVisitorsMeetingPasswordPageViewModel : BaseViewModel
	{
		#region Constructor.
		public VerifyInvitedVisitorsMeetingPasswordPageViewModel(INavigation navigation)
		{
		...
		private async Task ExecuteVerifyInterAppPasswordCommand()
		{
			if (!string.IsNullOrWhiteSpace(Password))
			{
				...
				{
					IVisitorsService visitorsService = new VisitorsService();
					await visitorsService.InformToEmployeeAsync(new Models.InformToEmployeeRequest()
					{
						UserId = Helper.Helper.UserId,
						Id = Helper.Helper.InvitedVisitorsMeetingId,
						VisitorProfileId = Helper.Helper.VisitorProfileId,
						IsCompany = Helper.Helper.UserId.Equals(Helper.Helper.VisitorProfileId) ? 0 : 1
					});
					...
					if (Device.RuntimePlatform == Device.iOS)
					{
						...
						if (Helper.Helper.AppToAppName.Equals("DstrictsApp"))
							...
							openAppUrl = $"NoffaPlusAppUrl://session/{response.Session}";
