...
{
	public class ConfirmUserInvitationInfoPasswordPageViewModel : BaseViewModel
	{
		#region Constructor.
		public ConfirmUserInvitationInfoPasswordPageViewModel(INavigation navigation)
		{
		...
		private async Task ExecuteVerifyInterAppPasswordCommand()
		{
			if (!string.IsNullOrWhiteSpace(Password))
			{
				...
				else if (response.Result == 1)
				{
					...
					await hotelService.ConfirmUserInvitationInfoAsync(new Models.ConfirmUserInvitationInfoRequest()
					{
						Id = ConfirmUserInvitationCode
					});
		...
		public string ConfirmUserInvitationCode { get; set; }
		#endregion
	}
}
