...
{
	public class VerifyPayForPayOnPasswordPageViewModel : BaseViewModel
	{
		#region Constructor.
		public VerifyPayForPayOnPasswordPageViewModel(INavigation navigation)
		{
		...
		private async Task ExecuteVerifyInterAppPasswordCommand()
		{
			if (!string.IsNullOrWhiteSpace(Password))
			{
				...
				else if (response.Result == 1)
				{
					...
					int cartServiceResponse = await cartService.PayCartItemUsingAppAsync(new Models.PayCartItemRequest()
					{
						CheckId = Helper.Helper.PayOnRequest.CheckedInId,
						ServiceType = Helper.Helper.PayOnRequest.ServiceType,
						QloudIdPay = Helper.Helper.PayOnRequest.QloudIdPay,
